...
/**
 * A utility class for Base64 encoding and decoding operations with support for both standard
 * and URL-safe variants.
 *
 ...
 * </ul>
 *
 * <p>The class supports optional padding with '=' characters and UTF-8 string
 * encoding/decoding.</p>
 *
 * <p>Example usage:</p>
 * <pre>
...
public final class Base64 {
     ...
     * This alphabet uses '~' (tilde) instead of '+' (plus) and '-' (hyphen) instead of '/' (slash)
     * compared to the standard Base64 alphabet.
    ...
    /**
     * The standard Base64 alphabet as defined in RFC 4648, Section 4.
     * This alphabet uses '+' (plus) and '/' (slash) characters.
     */
    private static final char[] BASE64_STANDARD_ALPHABET =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".toCharArray();

    /**
     * A reverse lookup table for the URL-safe Base64 alphabet ({@link #BASE64_ALPHABET}).
     * The table maps ASCII values of Base64 characters to their 6-bit integer equivalents.
     * Invalid characters are mapped to {@link #INVALID_CHAR}.
     */
    ...
    /**
     * Marker byte used in reverse lookup tables ({@link #BASE64_REVERSE},
     * {@link #BASE64_STANDARD_REVERSE}) to indicate that a character is not a valid Base64 character.
     * The value {@code (byte) 0xFF} is used, which is -1 in signed byte representation.
     */
     ...
     * No padding characters ('=') are appended to the output.
     *
     ...
     *
     * @param in        the byte array to encode. Must not be {@code null}.
     * @param equalsPad if {@code true}, padding characters ('=') are appended to the output
     *                  to ensure the encoded string length is a multiple of 4.
     *                  If {@code false}, no padding is added.
    ...
    /**
     * Encodes the given string into a Base64 string using the URL-safe alphabet and UTF-8 character encoding.
     * No padding characters ('=') are appended to the output.
     *
    ...
    /**
     * Encodes the given string into a Base64 string using the standard RFC 4648 alphabet and UTF-8 character encoding.
     * Padding characters ('=') are always appended to the output to ensure the encoded string length is a multiple of 4.
     *
     * @param in the string to encode. Must not be {@code null}.
    ...
    /**
     * Encodes the given byte array into a Base64 string using the standard RFC 4648 alphabet.
     * Padding characters ('=') are always appended to the output to ensure the encoded string length is a multiple of 4.
     *
     * @param in the byte array to encode. Must not be {@code null}.
     * @return the standard Base64 encoded string with padding.
    ...
    /**
     * Decodes a Base64 encoded string (URL-safe alphabet) into a byte array.
     * This method expects a string encoded using the URL-safe alphabet ({@link #BASE64_ALPHABET}).
     * It handles strings with or without padding characters ('=').
     *
     * @param inStr the URL-safe Base64 encoded string to decode. Must not be {@code null}.
     * @return the decoded byte array.
     * @throws IllegalBase64Exception if the input string {@code inStr} contains invalid Base64 characters
     *                                or has an invalid length for Base64 encoding.
     * @see #BASE64_REVERSE
     */
    ...
    /**
     * Decodes a Base64 encoded string (URL-safe alphabet) into a UTF-8 string.
     * This method expects a string encoded using the URL-safe alphabet ({@link #BASE64_ALPHABET}).
     * It handles strings with or without padding characters ('=').
     *
     * @param inStr the URL-safe Base64 encoded string to decode. Must not be {@code null}.
    ...
    /**
     * Decodes a Base64 encoded string (standard RFC 4648 alphabet) into a byte array.
     * This method expects a string encoded using the standard alphabet ({@link #BASE64_STANDARD_ALPHABET}).
     * It handles strings with or without padding characters ('=').
     *
     ...
     * <p>
     * This method processes the input byte array in 3-byte chunks, converting each chunk into
     * 4 Base64 characters. If the input array length is not a multiple of 3, padding
     * (either with Base64 characters or '=') is handled according to {@code equalsPad}
     * and the Base64 specification.
     *
     * @param in        the byte array to encode. Must not be {@code null}.
     * @param equalsPad if {@code true}, '=' characters are used for padding to make the
     *                  output length a multiple of 4. If {@code false}, no '=' padding is added,
     *                  and the output may be shorter.
     * @param alphabet  the character array representing the Base64 alphabet to use for encoding
     *                  (e.g., {@link #BASE64_ALPHABET} or {@link #BASE64_STANDARD_ALPHABET}).
     ...
     * <p>
     * This method processes the input string in 4-character chunks, converting each chunk
     * back into 3 bytes. It handles optional '=' padding characters at the end of the string.
     *
     * @param inStr           the Base64 encoded string to decode. Must not be {@code null}.
     *                        It may or may not include trailing '=' padding characters.
     * @param reverseAlphabet the byte array mapping Base64 character ASCII values to their
     *                        6-bit integer equivalents (e.g., {@link #BASE64_REVERSE} or
     *                        {@link #BASE64_STANDARD_REVERSE}).
     *                        Invalid characters should map to {@link #INVALID_CHAR}.
     * @return the decoded byte array.
     * @throws IllegalBase64Exception if the input string {@code inStr} contains non-Base64 characters,
     *                                has an invalid length (e.g., a remainder of 1 after stripping padding),
     *                                or if an {@link ArrayIndexOutOfBoundsException} occurs due to an invalid
     *                                character indexing into {@code reverseAlphabet} (which implies an
     *                                invalid character not caught by the {@code INVALID_CHAR} check,
     *                                though robust reverse tables should prevent this).
     */
    private static byte[] decode(String inStr, byte[] reverseAlphabet)
        ...
        if (remainder == 1) {
            throw new IllegalBase64Exception(
                "Invalid Base64 encoded string length: " + inStr.length() +
        ...
        try {
            ...
            throw new IllegalBase64Exception(
                ...
                "Input may contain characters not in the expected Base64 alphabet.", e);
        }
    }

    /**
     * Decodes a full 4-character Base64 chunk into a 24-bit integer.
     * Each character represents 6 bits. 4 characters * 6 bits/char = 24 bits.
     *
     ...
     * @param reverseAlphabet the reverse lookup table (e.g., {@link #BASE64_REVERSE})
     *                        to map Base64 characters to their 6-bit integer values.
     ...
     * <p>
     * - A 2-character chunk (e.g., "XX" from "XX==") decodes to 1 byte (8 bits).
     *   The 2 chars provide 12 bits; the significant 8 bits form the byte.
     * - A 3-character chunk (e.g., "XXX" from "XXX=") decodes to 2 bytes (16 bits).
     *   The 3 chars provide 18 bits; the significant 16 bits form the two bytes.
     * <p>
     * The resulting integer is shifted left so that the significant bits are in the
     * most significant positions, aligning with how {@link #decode(String, byte[])}
     * extracts bytes (i.e., {@code chunk >> 16} and {@code chunk >> 8}).
     *
     * @param in              the character array containing the Base64 input string.
     ...
     * @param reverseAlphabet the reverse lookup table (e.g., {@link #BASE64_REVERSE})
     *                        to map Base64 characters to their 6-bit integer values.
     * @return an integer where the most significant bits represent the decoded data.
     *         For len=2, the top 8 bits (of 12 decoded bits) are significant.
     *         For len=3, the top 16 bits (of 18 decoded bits) are significant.
     *         The result is shifted: {@code (decoded_bits) << (6 * (4 - len))}.
     * @throws IllegalBase64Exception if any character in the chunk is not a valid Base64 character
     *                                according to the {@code reverseAlphabet}.
     */
    private static int decodePartialChunk(char[] in, int pos, int len, byte[] reverseAlphabet)
        ...
        // If len is 2 (12 bits), shift left by 12 (6 * (4-2)) to fill 24 bits like XXXXXX XXXXXX 000000 000000.
        // If len is 3 (18 bits), shift left by 6  (6 * (4-3)) to fill 24 bits like XXXXXX XXXXXX XXXXXX 000000.
