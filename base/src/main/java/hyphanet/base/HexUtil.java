 ...
 *
 * <ul>
 *   <li>Converting between byte arrays and hexadecimal strings
 *   <li>Manipulating BitSets with hexadecimal representations
 *   <li>Handling BigInteger conversions to/from hex format
 *   <li>Reading/writing BigIntegers to data streams
 * </ul>
 *
 * <p>Unless otherwise stated, the conventions follow the rules outlined in the Java Language
 * Specification for hexadecimal notation.
 *
 * @author syoung
 */
public class HexUtil {
  ...
  public static String bytesToHex(byte[] bs, int off, int length) {
    if (bs.length < off + length) {
      throw new IllegalArgumentException(
          String.format(
              "Total length: %d, offset: %d, requested length: %d", bs.length, off, length));
    }
  ...
  /**
   * Converts a hexadecimal string to a byte array, storing the result at a specified offset within
   * the output array.
   *
   * <p>If the input hexadecimal string {@code s} has an odd number of characters, a leading '0' is
   ...
   *
   * <p>The output byte array {@code bs} will be allocated with a size of {@code off + (1 +
   * s.length()) / 2}. The converted bytes from the hexadecimal string will be placed starting at
   * {@code bs[off]}.
   *
   * @param s the hexadecimal string to convert. Can be of odd or even length. Allowed characters
   *     are '0'-'9', 'a'-'f', 'A'-'F'.
   * @param off the starting offset in the resulting byte array where the converted bytes will be
   *     stored. This offset determines the number of leading zero bytes or pre-existing data before
  ...
  public static byte[] hexToBytes(String s, int off) {
    // Calculate the necessary length: `off` for prefix, `(s.length()+1)/2` for hex data.
  ...
  public static void hexToBytes(String s, byte[] out, int off)
    ...
    for (int i = 0; i < slen; i += 2) {
      ...
      out[off + i / 2] = (byte) (b1 << 4 | Byte.toUnsignedInt(b2));
    }
  }
   ...
   *
   * <p>Packs the bits in the BitSet into a byte array, considering the specified size starting from
  ...
  public static byte[] bitsToBytes(BitSet ba, int size) {
    ...
    if (sb != null) {
      logger
          .atDebug()
          .setMessage("bytes: {} returned from bitsToBytes({},{}): {} for {}")
          .addArgument(bytesAlloc)
          .addArgument(ba)
          .addArgument(size)
          .addArgument(() -> bytesToHex(b))
          .addArgument(sb)
          .log();
    }
    return b;
  ...
  /**
   * Reads bits from a byte array and sets them in a {@link BitSet}. Bits are read from the first
   * byte in the array, proceeding from the least significant bit (LSB) to the most significant bit
   * (MSB) within each byte, and then moving to the next byte.
   *
   ...
   *       <ul>
   *         <li>BitSet index 0 gets bit 0 of byte 0 (1)
   *         <li>BitSet index 1 gets bit 1 of byte 0 (0)
   *         <li>...
   *         <li>BitSet index 7 gets bit 7 of byte 0 (0)
   *       </ul>
   *   <li>Byte 1 (0x02 = 00000010 in binary):
   *       <ul>
   *         <li>BitSet index 8 gets bit 0 of byte 1 (0)
   *         <li>BitSet index 9 gets bit 1 of byte 1 (1)
   *       </ul>
   * </ul>
   *
   * Only {@code numBitsToRead} bits will be set in the {@code BitSet}. If the byte array {@code b}
   * contains fewer than {@code numBitsToRead} bits, all available bits from {@code b} will be read
   * and set.
   *
   * @param b the byte array to read from.
   * @param bitSetTarget the {@link BitSet} to write the bits to. Any existing bits may be
   *     overwritten.
   * @param numBitsToRead the total number of bits to read from the byte array and set in the {@code
   *     BitSet}. Bits will be set in {@code bitSetTarget} from index 0 up to {@code numBitsToRead -
   *     1}.
   */
  public static void bytesToBits(byte[] b, BitSet bitSetTarget, int numBitsToRead) {
    if (logger.isDebugEnabled()) {
      logger.debug(
          "bytesToBits(bytesToHex(b)={}, bitSetTarget, numBitsToRead={})",
          bytesToHex(b),
          numBitsToRead);
    }
  ...
  /**
   * Converts a hexadecimal string to a {@link BitSet}. The hexadecimal string is first converted to
   * a byte array, and then {@link #bytesToBits(byte[], BitSet, int)} is used to populate the {@code
   * BitSet}.
   *
   ...
   * @param numBitsToSet the total number of bits to take from the converted hex string and set in
   *     the {@code BitSet}. Bits will be set in {@code bitSetTarget} from index 0 up to {@code
   *     numBitsToSet - 1}. If the hex string represents fewer bits than {@code numBitsToSet}, all
   *     bits from the hex string will be set.
  ...
  /**
   * Writes a non-negative {@link BigInteger} to a {@link DataOutputStream}. The format used is:
   *
   * <ol>
   *   <li>The length of the BigInteger's byte array representation (obtained from {@link
   *       BigInteger#toByteArray()}) is written as a {@code short} (2 bytes).
   *   <li>The byte array itself is then written to the stream.
   * </ol>
   *
  ...
  public static void writeBigInteger(BigInteger integer, DataOutputStream out) throws IOException {
    ...
    if (buf.length > Short.MAX_VALUE) {
      throw new IllegalStateException(
          String.format(
              "BigInteger too long: %d bytes. Maximum allowed is %d.",
  ...
  /**
   * Reads a {@link BigInteger} from a {@link DataInputStream} that was written using the format
   * defined in {@link #writeBigInteger(BigInteger, DataOutputStream)}.
   *
   * <p>The format expected is:
   *
   * <ol>
   *   <li>A {@code short} indicating the length of the byte array for the BigInteger.
   *   <li>The byte array itself.
   * </ol>
   *
   * The method reconstructs the {@link BigInteger} assuming it represents a positive value (using
   * {@code new BigInteger(1, buf)}). This is consistent with {@link #writeBigInteger(BigInteger,
   * DataOutputStream)} which only allows non-negative integers.
   *
   * @param dis the {@link DataInputStream} to read from.
   * @return the {@link BigInteger} read from the stream.
   * @throws IOException if an I/O error occurs, if the end of stream is reached prematurely, or if
   *     the encoded length is negative (which might indicate data corruption or an incompatible
   *     format).
  ...
  public static BigInteger readBigInteger(DataInputStream dis) throws IOException {
    ...
    if (length < 0) {
      ...
      throw new IOException(
          String.format(
              "Invalid BigInteger length read from stream: %d. Length must be non-negative.",
              length));
    }
   ...
   * Converts a BigInteger to a hexadecimal string.
   *
   * <p>This method is provided as an alternative to BigInteger.toString(16) which had issues with
   * NPE on some JDK versions.
   *
