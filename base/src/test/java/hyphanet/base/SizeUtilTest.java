...
class SizeUtilTest {
  ...
  @Test
  void testFormatSizeLong() {
    long methodLong;
    methodLong = Long.parseLong(valAndExpected[0][0]);
    assertEquals(SizeUtil.formatSize(methodLong), "1 " + valAndExpected[0][1]);

    for (int i = 1; i < valAndExpected.length; i++) {
      methodLong = Long.parseLong(valAndExpected[i][0]);
   ...
   */
  @Test
  void testFormatSizeLong_WithIntermediateValues() {
    long methodLong;
