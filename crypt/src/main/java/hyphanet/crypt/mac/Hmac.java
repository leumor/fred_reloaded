...
/**
 * Implements the HMAC (Hash-based Message Authentication Code) function as described in <a
 * href="https://csrc.nist.gov/publications/detail/fips/198/1/final">FIPS PUB 198-1</a>.
 *
 * <p>This implementation provides a secure way to create and verify message authentication codes
...
public enum Hmac {
  ...
  public static byte[] mac(Hmac hash, final byte[] key, final byte[] data) {
    if (key.length != hash.digestSize) {
      throw new IllegalArgumentException(
          String.format(
              "Invalid key size: expected %d bytes, got %d bytes", hash.digestSize, key.length));
    }
  ...
  /**
   * The standard algorithm name for the HMAC implementation. This name must match the algorithm
   * names defined in the Java Security Standard Algorithm Names Specification.
   */
  final String algo;

  /**
   * The size of the message digest produced by this HMAC algorithm, in bytes. For SHA-256, this is
   * 32 bytes (256 bits).
   */
