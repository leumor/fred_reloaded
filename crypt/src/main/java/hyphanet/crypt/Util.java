 ...
 * manipulations, and cipher operations.
 *
 * <p>This class is part of Hyphanet's cryptographic infrastructure and provides:
 *
 * <ul>
 *   <li>Hash calculations using different algorithms (SHA1, MD5, SHA256, etc.)
 *   <li>Byte array conversion utilities
 *   <li>MPI (Multi-Precision Integer) operations
 *   <li>Key generation and cipher operations
 * </ul>
 */
public class Util {
  ...
  /**
   * Fills a byte array from an array of integers. Each integer is split into 4 bytes in big-endian
   * order.
   *
  ...
  /**
   * Fills a byte array from an array of longs. Each long is split into 8 bytes in big-endian order.
   *
   ...
   *
   * <p>The MPI format consists of:
   *
   * <ul>
   *   <li>2 bytes for the bit length
   *   <li>The actual number data in big-endian format
   * </ul>
   *
   ...
   * @throws IllegalStateException if the specified algorithm is not available
   */
  public static byte[] hashBytes(HashAlgorithm algorithm, byte[] data, int offset, int length) {
  ...
  public static void makeKey(byte[] entropy, byte[] key, int offset, int len) {
    try {
      MessageDigest digest = MessageDigest.getInstance(HashAlgorithm.SHA1.getAlgorithmName());
      int ic = 0;
      while (len > 0) {
        ic++;
        for (int i = 0; i < ic; i++) {
  ...
  /**
   * Fully reads the requested number of bytes from an input stream into a buffer at the specified
   * offset.
   *
   * @param in The input stream to read from
   * @param b The buffer into which the data is read
  ...
  /**
   * Generates a random positive BigInteger with a specified number of bits using a provided random
   * number generator.
   *
   * <p>This method creates a random BigInteger that is uniformly distributed over the range [0,
   * 2^numBits - 1]. The generated number will have exactly the specified number of bits.
   *
