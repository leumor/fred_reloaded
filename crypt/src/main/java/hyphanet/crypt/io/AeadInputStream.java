 ...
 * Associated Data (AEAD). This class reads encrypted data and verifies its integrity upon
 * closing. <b>Important:</b> Authentication occurs during stream closure - callers must ensure
 * {@link #close()} is executed even if exceptions occur.
 * <p>
 * Uses OCB (Offset Codebook Mode) with two AES ciphers: one for bulk data processing and
 * another for lightweight MAC computation. The nonce is read from the stream header.
 */
public class AeadInputStream extends FilterInputStream {
     ...
     * <p>
     * <b>Implementation Note:</b> Uses temporary buffers during decryption as AEAD ciphers
     * may produce more/fewer bytes than input. Excess decrypted bytes are cached for
     * subsequent reads.
     *
    ...
    /**
     * Decrypts data into the buffer, prioritizing cached excess bytes before reading new
     * blocks.
     *
    ...
    public int read(byte[] buf, int offset, int length) throws IOException {
        ...
        if (outLength > length) {
            ...
            assert decryptedBytes == outLength :
                "Decrypted bytes mismatch expected output size";
    ...
    /**
     * @return Estimated available bytes including excess buffer and underlying stream. Note:
     * Accuracy is limited as MAC bytes may not be fully read yet.
     */
    @Override
    ...
    /**
     * Skips bytes by decrypting and discarding them. Required for MAC validation during
     * close().
     *
    ...
    /**
     * Closes the stream and enforces full data consumption to validate MAC.
     * <b>Must be called</b> to ensure authentication, even if exceptions occur.
     */
    @Override
