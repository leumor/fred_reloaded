...
/**
 * An {@link OutputStream} that encrypts and authenticates data using Authenticated Encryption with
 * Associated Data (AEAD) in OCB (Offset Codebook) mode, typically with AES ciphers. This class
 * writes a unique nonce to the stream upon initialization, which is required for decryption.
 *
 * <p>The implementation uses Bouncy Castle's {@link AEADBlockCipher} for encryption and
 * authentication. Data is processed in blocks, and the final MAC (Message Authentication Code) is
 * appended upon closing the stream. The overhead per encrypted block includes the AES block size
 * and MAC size.
 *
 * @author toad
 */
public class AeadOutputStream extends FilterOutputStream {
   ...
   * @param key Encryption key (must match cipher requirements, e.g., 16/24/32 bytes for AES).
   * @param nonce Nonce (IV) for encryption. Must be unique and have the top bit of the first byte
   *     set to 0. We will write it to the stream so the other side can pick it up, like an IV.
   *     Should generally be generated from a SecureRandom.
   * @param hashCipher Block cipher used for MAC computation, not a block mode (e.g., {@link
   *     AESLightEngine}). This will not be used very much.
   * @param mainCipher Block cipher used for data encryption, not a block mode (e.g., {@link
   *     AESEngine}). This will be used for encrypting a fairly large amount of data so could be any
   *     of the 3 BC AES impl's.
  ...
  /**
   * Encrypts a single byte and writes it to the stream. Internally buffers the byte and delegates
   * to {@link #write(byte[], int, int)}.
   */
  @Override
   ...
   *
   * <p>Data is processed through the AEAD cipher, producing encrypted bytes and intermediate MAC
   * values. The encrypted bytes are written immediately, but the final MAC is only appended upon
   * calling {@link #close()}.
   *
  ...
  /**
   * Finalizes encryption, appends the final MAC to the stream, and closes the underlying stream.
   * After this, no more data can be written.
   *
   * @throws IOException If closing the underlying stream fails.
   * @throws IllegalStateException If cipher finalization fails (should never happen in encryption).
   */
  @Override
  ...
  /** For unit tests only: Creates an instance with configurable Random (not necessarily secure). */
