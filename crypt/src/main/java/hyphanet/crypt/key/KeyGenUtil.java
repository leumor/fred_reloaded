...
/**
 * A utility class providing cryptographic key generation and manipulation functionality. This class
 * offers methods to generate and convert various types of cryptographic keys, initialization
 * vectors (IVs), and nonces.
 *
 * <p>Key features include:
 *
 * <ul>
 *   <li>Generation of public/private key pairs
 *   <li>Conversion between different key formats
 *   <li>Secret key generation and manipulation
 *   <li>IV and nonce generation
 *   <li>Key derivation functions
 * </ul>
 *
 * <p><strong>Note:</strong> This class cannot handle DSA keys and will throw exceptions for
 * DSA-related operations.
 *
 * @author unixninja92
...
public final class KeyGenUtil {
   ...
   *
   * @param type the {@link KeyPairType} specifying the algorithm format for key pair generation
   ...
   * @throws SecurityException if key pair generation fails due to algorithm unavailability or
   *     invalid parameters
   */
  ...
  /**
   * Converts a specified key for a specified algorithm to a {@link PublicKey} which is then stored
   * in a {@link KeyPair}. The private key of the KeyPair is null.
   *
   ...
   * in a {@link KeyPair}. The private key of the KeyPair is null.
   *
   * <p>This method first converts the {@link ByteBuffer} input to a byte array, then processes it
   * to create a public key. The resulting {@link KeyPair} will have a null private key component.
   *
   * @param type the {@link KeyPairType} specifying the algorithm format
   * @param pub the public key as a {@link ByteBuffer}
   * @return a {@link KeyPair} containing the public key and a null private key
  ...
  /**
   * Converts the specified keys for a specified algorithm to {@link PrivateKey} and {@link
   * PublicKey} respectively, storing them in a {@link KeyPair}.
   *
   * @param type the {@link KeyPairType} specifying the algorithm format
   ...
   * PublicKey} respectively, storing them in a {@link KeyPair}.
   *
   * <p>This method first converts the {@link ByteBuffer} inputs to byte arrays, then processes them
   * to create the appropriate key objects.
   *
   * @param type the {@link KeyPairType} specifying the algorithm format
   * @param pub the public key as a {@link ByteBuffer}
   ...
   *
   * <p><strong>Note:</strong> For HMAC keys, any key length is acceptable. For other algorithms,
   * the key length must match the specified key size of the algorithm.
   *
   ...
   * @throws IllegalArgumentException if the key length doesn't match the required size for non-HMAC
   *     algorithms
   */
  public static SecretKey getSecretKey(KeyType type, byte[] key) {
    if (!type.name().startsWith("HMAC") && key.length != type.keySize >> 3) {
      throw new IllegalArgumentException(
          String.format(
              "Key size %d does not match " + "required size %d", key.length, type.keySize >> 3));
    }
  ...
  /**
   * Generates a cryptographically secure initialization vector (IV) of the specified length.
   *
  ...
  /**
   * Creates an {@link IvParameterSpec} from initialization vector data stored in a {@link
   * ByteBuffer}.
   *
  ...
  /**
   * Derives an initialization vector (IV) using a key derivation function based on HMAC-SHA-512.
   *
   * @param kdfKey the base key used for derivation
   * @param c the class whose name will be used in the derivation process
   * @param kdfString additional string input for the derivation process
