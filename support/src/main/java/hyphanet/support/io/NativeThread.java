...
public class NativeThread extends Thread {
    ...
    static {
        ...
        if (maybeLoadNative) {
            ...
            System.out.println("Using the NativeThread implementation (base nice level is " +
			...
			if (!(HAS_ENOUGH_NICE_LEVELS && HAS_THREE_NICE_LEVELS)) {
				System.err.println(
					"WARNING!!! The JVM has been niced down to a level which won't allow it " +
					"to schedule threads properly! LOWER THE NICE LEVEL!!");
			}
            _loadNative = true;
        } else {
            // unused anyway
    ...
    private boolean setNativePriority(int prio) {
        ...
        if (NATIVE_PRIORITY_BASE != realPrio && !dontCheckRenice) {
            /* The user has reniced freenet or we didn't use the PacketSender to create the
            thread
            ...
            _disabled = true;
            Logger.error(this,
                         "Freenet has detected it has been reniced : THAT'S BAD, DON'T DO " +
						 "IT! Nice level detected statically: " +
                         NATIVE_PRIORITY_BASE + " actual nice level: " + realPrio + " on " +
                         this);
            System.err.println(
                "Freenet has detected it has been reniced : THAT'S BAD, DON'T DO IT! Nice " +
        ...
        final int linuxPriority = NATIVE_PRIORITY_BASE + NATIVE_PRIORITY_RANGE -
                                  (NATIVE_PRIORITY_RANGE * (prio - MIN_PRIORITY)) /
                                  JAVA_PRIORITY_RANGE;
		...
		if (prio < currentPriority) {
			throw new IllegalStateException("You're trying to set a thread priority" +
											" above the current value!! It's not possible if" +
											" you aren't root" +
											" and shouldn't ever occur in our code. (asked=" +
											prio + ':' + linuxPriority + " currentMax=" +
											currentPriority + ':' + NATIVE_PRIORITY_BASE +
											") SHOULD NOT HAPPEN, please report!");
		}
        Logger.minor(this, "Setting native priority to " + linuxPriority + " (base=" +
                           NATIVE_PRIORITY_BASE + ") for " + this);
