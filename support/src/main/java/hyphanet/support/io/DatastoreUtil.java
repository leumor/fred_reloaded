...
public class DatastoreUtil {
    ...
    public static long maxDatastoreSize() {
        ...
        if (maxMemory == Long.MAX_VALUE || maxMemory < 128 * oneMiB) {
            ...
            // There are 3 types of keys. We want the number of { SSK, CHK, pubkey } i.e.
            // the number of slots in each store.
            slots /= 3;
            // We return the total size, so we don't need to worry about cache vs store or
            // even client cache.
            // One key of all 3 types combined uses Node.sizePerKey bytes on disk. So we get
            // a size.
            maxDatastoreSize =
                slots * Node.sizePerKey; // in total this is (RAM - 100 MiB) / 24 * ~32 KiB
        }
    ...
    public static long autodetectDatastoreSize(NodeClientCore core, Config config) {
        ...
        if (freeSpace <= 0) {
            ...
            if (freeSpace > 50 * oneGiB) {
                // > 50 GiB: Use 10% free space; minimum 10 GiB. Limited by bloom filters
                // and disk I/O.
                shortSize = Math.max(10 * oneGiB, Math.min(freeSpace / 10, Math.min(diskIoMax,
