...
/**
 * An implementation of {@link RandomAccessBuffer} that stores data in a byte array in memory.
 * This class provides thread-safe random access operations through synchronization.
 * <p>
 * The buffer can be set to read-only mode, after which write operations will throw an
 * {@link IOException}. All operations are synchronized to ensure thread safety.
 * </p>
 *
 * @see RandomAccessBuffer
 */
public class ByteArray implements RandomAccessBuffer, Serializable {
     ...
     */
    public ByteArray(byte[] initialContents, int offset, int size, boolean readOnly) {
        if (offset < 0 || size < 0 || offset + size > initialContents.length) {
            throw new IllegalArgumentException(
                "Invalid parameters: offset=%d, size=%d, array length=%d".formatted(
                    offset,
                    size,
     ...
     * {@inheritDoc}
     * <p>
     * Marks this buffer as closed. Further operations will fail.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * Performs a synchronized read operation from the internal buffer.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * Performs a synchronized write operation to the internal buffer.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * Returns a dummy lock as this implementation is always open.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * No-op as no resume action is needed.
     */
    @Override
    ...
    private void validateReadOperation(long fileOffset, int length) throws IOException {
        ...
        if (fileOffset + length > data.length) {
            throw new IOException(
                "Cannot read after end: trying to read from %d to %d on block length %d".formatted(
                    fileOffset,
                    fileOffset + length,
                    data.length
                ));
     ...
     *
     * @throws IOException              if the write operation would exceed buffer bounds or
     *                                  buffer is read-only
     * @throws IllegalArgumentException if fileOffset is negative
     */
