...
/**
 * Trivial random access file base interface. Guaranteed to be thread-safe - that is, either
 * the implementation will serialise reads, or it will support parallel reads natively. The
 * length of the file is constant. Many implementations will provide a nulling constructor -
 * one that takes a size and creates a RandomAccessBuffer of that length whose content is all
 * 0's.
 *
 * @author toad
 */
public interface RandomAccessBuffer extends Closeable {
     ...
     *
     * @throws IOException              If we were unable to read the required number of bytes
     *                                  etc.
    ...
    /**
     * Free the underlying resources. May do nothing in some implementations. You should make
     * sure the object can be GC'ed as well.
     */
    void free();
