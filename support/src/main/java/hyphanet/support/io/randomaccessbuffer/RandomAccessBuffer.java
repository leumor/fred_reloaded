 ...
 *
 * <p>
 * This interface provides basic random access operations for reading and writing data at
 * specific positions in a file-like structure. All implementations must guarantee
 * thread-safety either through serialization of reads or native parallel read support.
 * </p>
 *
 * <p>This interface allows temporary locking of the underlying resource to prevent
 * premature closure of file descriptors, especially in pooled scenarios. While locking doesn't
 * provide explicit concurrency guarantees, implementations must ensure thread-safety through
 * appropriate synchronization mechanisms. Implementations must register with
 * {@link BucketTools#restoreRabFrom(DataInputStream, FilenameGenerator, PersistentFileTracker,
 * MasterSecret)}.</p>
 *
 * @author toad
 * @since 1.0
 */
public interface RandomAccessBuffer extends AutoCloseable {
    ...
    abstract class RabLock {
        ...
        /**
         * Template method for implementing specific unlock behavior. Implementations should
         * handle resource cleanup and state management.
         */
     ...
     * <p>
     * This method guarantees to read the entire requested range or throw an exception, similar
     * to {@link java.io.DataInputStream#readFully(byte[], int, int)}. The operation will fail
     ...
     * <p>
     * This method writes the specified range of bytes from the source array to the buffer at
     * the given offset.
     * </p>
     *
     ...
     *
     * <p><strong>Note:</strong> This method may block until a slot becomes available,
     * which could potentially lead to deadlocks if not used carefully.</p>
     *
     ...
     *
     * <p>This method is called after deserialization to perform necessary
     * initialization steps, such as registering with temporary file management systems.</p>
     *
     ...
     *
     * <p>Implementations must write a unique magic value for proper reconstruction
     * during deserialization, and add a clause to
     * {@link BucketTools#restoreRabFrom(DataInputStream, FilenameGenerator,
     * PersistentFileTracker, MasterSecret)}. The serialization format should be versioned if
     * necessary.</p>
     *
     ...
     *
     * <p>Implementations must override this method to properly compare RABs,
     * especially during splitfile insert resume operations.</p>
     *
