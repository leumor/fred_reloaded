 ...
 * files. It produces {@link PooledFile} instances, which are {@link RandomAccessBuffer}
 ...
 * @see Factory
 */
public class PooledFileFactory implements Factory {

  /**
   * Constructs a new {@link PooledFileFactory} with the specified {@link FilenameGenerator}.
   *
   ...
   */
  public PooledFileFactory(FilenameGenerator filenameGenerator) {
  ...
  public RandomAccessBuffer makeRab(long size) throws IOException {
    ...
    RandomAccessBuffer ret = null;
    try {
      ret = new PooledFile(path, false, size, id, true);
      return ret;
    } finally {
      if (ret == null) {
        Files.delete(path);
  ...
  public RandomAccessBuffer makeRab(byte[] initialContents, int offset, int size, boolean readOnly)
    ...
    RandomAccessBuffer ret = null;
    try {
      ret = new PooledFile(path, initialContents, offset, size, id, true, readOnly);
      return ret;
    } finally {
      if (ret == null) {
        Files.delete(path);
