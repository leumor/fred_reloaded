...
abstract class SwitchableProxyRandomAccessBuffer implements LockableRandomAccessBuffer {
     ...
     */
    protected abstract LockableRandomAccessBuffer innerMigrate(
        LockableRandomAccessBuffer underlying) throws IOException;
    /**
     * Size of the temporary storage. Note that this may be smaller than underlying.size(), and
     * will be enforced before passing requests on.
     */
    final long size;
    /**
    ...
    /**
     * Number of currently valid RAFLock's on this RAF. We centralise this here so that we only
     * have to take a single new lock when migrating.
     */
