...
/**
 * <p>A specialized {@link FilterInputStream} implementation that provides line reading
 * capabilities
 ...
 *   <li>Built-in protection against memory exhaustion through configurable line length
 *   limits</li>
 *   <li>Optimized reading strategies based on underlying stream capabilities (mark/reset
 *   support)</li>
 * </ul>
 *
...
public class LineReadingInputStream extends FilterInputStream implements LineReader {
    ...
    public @Nullable String readLine(int maxLength, int bufferSize, boolean utf)
        ...
        if (maxLength <= bufferSize) {
            bufferSize =
                maxLength + 1; // Buffer too big, shrink it (add 1 for the optional \r)
        }
     ...
     *
     * <p>This method reads the input stream byte by byte until one of the following conditions
     * is
     * met:</p>
     * <ul>
     ...
     */
    protected @Nullable String readLineWithoutMarking(
        int maxLength, int bufferSize, boolean utf) throws IOException {
    ...
    /**
     * <p>Calculates the optimal buffer size based on the given constraints, balancing memory
     * usage
     * and performance.</p>
     *
    ...
    private static TooLongException createTooLongException(
        ...
        return new TooLongException(
            String.format("Line exceeded maximum length of %d bytes%n%s%n%s", maxLength,
                          HexUtil.bytesToHex(buf, 0, ctr), new String(buf, 0, ctr, utf ?
