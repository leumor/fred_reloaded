...
/**
 * A proxy Bucket which adds: - Encryption with the supplied cipher, and a random, ephemeral
 * key. - Padding to the next PO2 size.
 * <p>
...
public class PaddedEphemerallyEncryptedBucket implements Bucket, Serializable {
     ...
     *                   responsibility to free the random number generators, but we WILL free
    ...
    public PaddedEphemerallyEncryptedBucket(
        PaddedEphemerallyEncryptedBucket orig, Bucket newBucket) {
    ...
    public static long paddedLength(long dataLength, long minPaddedSize) {
        ...
        while (true) {
            ...
            if ((size >= min) && (size <= max)) {
				if (logMINOR) {
					Logger.minor(PaddedEphemerallyEncryptedBucket.class,
        ...
        public void close() throws IOException {
            try {
                ...
                synchronized (PaddedEphemerallyEncryptedBucket.this) {
                    ...
                    if (streamNumber != lastOutputStream) {
                        Logger.normal(this,
                                      "Not padding out to length because have been " +
									  "superceded: " +
                                      getName());
                        return;
                    }
                    ...
                    while (writtenPadding < padding) {
                        int left = (int) Math.min((long) (padding - writtenPadding),
                                                  (long) buf.length);
