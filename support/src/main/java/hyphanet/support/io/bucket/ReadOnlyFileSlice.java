...
/**
 * Provides a read-only view of a slice of a file, representing a continuous portion of the
 * file from a specified starting position with a defined length. This implementation is
 * immutable and thread-safe.
 *
 * <p>The slice maintains a view into the underlying file without loading the entire content
 * into memory, making it memory efficient for large files.</p>
 *
 * <p><b>Thread Safety:</b> This class is thread-safe. Multiple threads can safely read
 * from the same instance concurrently.</p>
 *
 * <p><b>Note:</b> The underlying file must exist and remain unchanged during the lifetime
 * of this slice. If the file is modified or deleted, subsequent operations may fail.</p>
 * <p>
 * FIXME: implement a hash verifying version of this.
 *
 * @see Bucket
 */
public class ReadOnlyFileSlice implements Bucket, Serializable {
     ...
     *
     * @throws IOException              if the file cannot be accessed or if the slice
     *                                  parameters are invalid
     * @throws IllegalArgumentException if startAt or length are negative or if the slice
     *                                  extends beyond the file bounds
     */
    public ReadOnlyFileSlice(Path path, long startAt, long length) throws IOException {
     ...
     */
    protected ReadOnlyFileSlice(DataInputStream dis)
    ...
    public Bucket createShadow() {
        try {
            return new ReadOnlyFileSlice(path, startAt, length);
        } catch (IOException e) {
            return null;
        }
    }
     ...
     */
    private static void validateParameters(Path path, long startAt, long length)
        throws IOException {
        if (startAt < 0) {
    ...
    /**
     * Provides stream access to the file slice. This implementation ensures that only the
     * specified portion of the file is accessible through the stream.
     */
    private class SliceInputStream extends InputStream {
        ...
        /**
         * The channel for reading from the underlying file. Provides random access
         * capabilities for efficient slice reading.
         */
        ...
        /**
         * The current position within the slice, relative to startAt. Used to track read
         * progress and enforce slice boundaries.
         */
        private long position;
    }

    /**
     * The starting position of this slice in the file. This value must be non-negative and
     * within the file bounds.
     */
    private final long startAt;

    /**
     * The length of this slice in bytes. This value must be non-negative and the slice must
     * not extend beyond the file bounds.
     */
    private final long length;

    /**
     * The path to the underlying file. This field is transient as Path is not serializable,
     * and is reconstructed during deserialization.
     */
