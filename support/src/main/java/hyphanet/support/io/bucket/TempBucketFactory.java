 ...
 * <p>
 * ArrayBuckets are used if and only if: 1) there is enough room remaining on the pool (@see
 ...
 * <p>
 * Currently they are two factors considered for a migration: - if they are long-lived or not
 * (@see RAMBUCKET_MAX_AGE) - if their size is over
...
public class TempBucketFactory implements BucketFactory, LockableRandomAccessBufferFactory {
    ...
    public TempBucketFactory(
        Executor executor, FilenameGenerator filenameGenerator, long maxBucketSizeKeptInRam,
        long maxRamUsed, Random weakPRNG, boolean reallyEncrypt, long minDiskSpace,
        ...
        this.diskRAFFactory =
            new DiskSpaceCheckingRandomAccessBufferFactory(underlyingDiskRAFFactory,
    ...
    public class TempBucket implements Bucket, Migratable, RandomAccessBucket {
        ...
        public synchronized InputStream getInputStreamUnbuffered() throws IOException {
            if (!hasWritten) {
                throw new IOException(
                    "No OutputStream has been openned! Why would you want an InputStream " +
                    "then?");
            }
            if (hasBeenFreed) {
                throw new IOException("Already freed");
        ...
        public LockableRandomAccessBuffer toRandomAccessBuffer() throws IOException {
            synchronized (this) {
                ...
                TempRandomAccessBuffer raf =
                    new TempRandomAccessBuffer(currentBucket.toRandomAccessBuffer(),
        ...
        protected void finalize() throws Throwable {
            ...
            if (!hasBeenFreed) {
                if (TRACE_BUCKET_LEAKS) {
                    Logger.error(this,
                                 "TempBucket not freed, size=" + size() + ", isRAMBucket=" +
                                 isRAMBucket() + " : " + this, tracer);
                } else {
                    Logger.error(this,
                                 "TempBucket not freed, size=" + size() + ", isRAMBucket=" +
                                 isRAMBucket() + " : " + this);
        ...
        private class TempBucketOutputStream extends OutputStream {
            ...
            private void _maybeMigrateRamBucket(long futureSize) throws IOException {
                ...
                if (isRAMBucket()) {
                    ...
                    if (futureSize >= Math.min(Integer.MAX_VALUE, maxRAMBucketSize *
                        ...
                        if (logMINOR) {
                            if (isOversized) {
                                Logger.minor(this,
                                             ...
                                             SizeUtil.formatSize(maxRAMBucketSize *
                                                                 RAMBUCKET_CONVERSION_FACTOR) +
                                             ": we will force-migrate it to disk.");
                            } else {
                                Logger.minor(this,
                                             "The bucketpool is full: force-migrate before " +
                    ...
                    if (futureSize - lastCheckedSize >= CHECK_DISK_EVERY) {
                        if (filenameGenerator.getDir().getUsableSpace() +
                            (futureSize - currentSize) < minDiskSpace) {
        ...
        private class TempBucketInputStream extends InputStream {
            ...
            /**
             * Keep a counter to know where we are on the stream (useful when we have to reset
             * and skip)
             */
        ...
        /**
         * We have to account the size of the underlying bucket ourself in order to be able to
         * access it fast
         */
     ...
     * @param force If true, migrate one bucket which isn't necessarily long lived, just to
     *              free up space. Otherwise we will migrate all long-lived buckets but not any
     *              others.
     *
     * @return True if we migrated any buckets.
     *
     * @throws InsufficientSpaceException If there is not enough space to migrate buckets to
     *                                    disk.
     */
            ...
            synchronized (ramBucketQueue) {
                ...
                if (tmpBucketRef == null) {
                    ...
                    if (tmpBucket.creationTime() + RAMBUCKET_MAX_AGE > now && !force) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this, "The bucket " + tmpBucket + " is " +
                                               TimeUtil.formatTime(
     ...
     * thread.
     */
    class TempRandomAccessBuffer extends SwitchableProxyRandomAccessBuffer
        implements Migratable {

        public TempRandomAccessBuffer(
            byte[] initialContents, int offset, int size, long time, boolean readOnly)
            throws IOException {
            super(new ByteArrayRandomAccessBuffer(initialContents, offset, size, readOnly),
                  size);
            creationTime = time;
            hasMigrated = false;
            original = null;
        ...
        protected LockableRandomAccessBuffer innerMigrate(
            LockableRandomAccessBuffer underlying) throws IOException {
        ...
        protected void finalize() throws Throwable {
            ...
            if (!hasBeenFreed()) {
                if (TRACE_BUCKET_LEAKS) {
                    Logger.error(this,
                                 "TempRandomAccessBuffer not freed, size=" + size() + " : " +
                                 this, tracer);
                } else {
                    Logger.error(this,
                                 "TempRandomAccessBuffer not freed, size=" + size() + " : " +
         ...
         * TempRandomAccessBuffer *and* the TempBucket are no longer reachable, in which case
         * we will free from the TempBucket. If this is null, then the TempRAB can free in
         * finalizer.
         */
            ...
            try {
                ...
                while (true) {
                    try {
                        ...
                        if (!saidSo) {
                            Logger.error(this,
                                         "Insufficient disk space to migrate in-RAM buckets " +
                                         "to disk!");
