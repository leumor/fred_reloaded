...
/**
 * Pads a bucket to the next power of 2 file size. Note that self-terminating formats do not
 * work with AEADCryptBucket; it needs to know the real length. This pads with FileUtil.fill(),
 * which is reasonably random but is faster than using SecureRandom, and vastly more secure
 * than using a non-secure Random.
 */
public class PaddedRandomAccessBucket implements RandomAccessBucket, Serializable {
    ...
    /**
     * Create a PaddedBucket, specifying the actual size of the existing bucket, which we do
     * not store on disk.
     *
    ...
    @Override
    /** Get the size of the data written to the bucket (not the padded size). */ public synchronized long size() {
        return size;
    }
