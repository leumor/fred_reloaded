...
public class BucketTools {
     ...
     * @throws OutOfMemoryError If it was not possible to allocate enough memory to contain the
     *                          entire bucket.
     */
    ...
    public static RandomAccess makeImmutableBucket(Factory bucketFactory, byte[] data)
        throws IOException {
        return makeImmutableBucket(bucketFactory, data, data.length);
    }

    public static RandomAccess makeImmutableBucket(
        Factory bucketFactory, byte[] data, int length) throws IOException {
    ...
    public static RandomAccess makeImmutableBucket(
        ...
        RandomAccess bucket = bucketFactory.makeBucket(length);
            ...
            try {
                ...
                if ((bytesRead != bucketLength) && (bucketLength > 0)) {
                    throw new IOException(
                        "Read " + bytesRead + " but bucket length " + bucketLength + " on " +
                        data + '!');
                }
            ...
            while (moved < truncateLength) {
                ...
                if (bytes <= 0) {
                    throw new IllegalStateException(
                        "bytes=" + bytes + ", truncateLength=" + truncateLength + ", moved=" +
                        moved);
                }
                ...
                if (bytes <= 0) {
                    ...
                    IOException ioException = new IOException(
                        "Could not move required quantity of data in copyTo: " + bytes +
                        " (moved " + moved + " of " + truncateLength +
        ...
        try {
            ...
            while (moved < truncateLength) {
                ...
                if (bytes <= 0) {
                    ...
                    IOException ioException = new IOException(
                        "Could not move required quantity of data in copyFrom: " + bytes +
                        " (moved " + moved + " of " + truncateLength +
                        "): unable to read from " + is);
                    ioException.printStackTrace();
                    throw ioException;
     ...
     *                   <p>
     *                   If the passed-in Bucket is a FileBucket, will be efficiently split
     *                   into ReadOnlyFileSliceBuckets, otherwise new buckets are created and
     ...
     *                   ensuring that the returned buckets HAVE ALREADY BEEN STORED TO THE
     *                   DATABASE, using the provided handle. The point? SegmentedBCB's buckets
     *                   have already been stored!!
     *
     * @throws IOException If there is an error creating buckets, reading from the provided
    ...
    public static long copyTo(
        Bucket bucket, RandomAccessBuffer raf, long fileOffset,
