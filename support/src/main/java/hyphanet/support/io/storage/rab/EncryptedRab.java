 ...
 * SkippingStreamCipher. This class encrypts data using ChaCha stream cipher and provides integrity
 * verification through MAC. The encrypted data is stored in an underlying RandomAccessBuffer along
 * with a header containing encryption metadata.
 *
 * <h2>Security Features:</h2>
 *
 * <ul>
 *   <li>Encryption using ChaCha stream cipher (128/256 bit)
 *   <li>Message Authentication Code (MAC) for integrity verification
 *   <li>Secure key derivation from master secret
 *   <li>Thread-safe read/write operations
 * </ul>
 *
 * @author unixninja92
 * @see Rab
...
public final class EncryptedRab extends AbstractStorage implements Rab, Serializable {
  ...
  /**
   * Enumeration of key derivation function inputs used to derive encryption and IV keys from the
   * unencrypted base key.
   */
  public enum KdfInput {
    /** Used for deriving the key that encrypts the underlying RandomAccessBuffer */
    UNDERLYING_KEY(),

    /** Used for deriving the initialization vector for underlying RandomAccessBuffer encryption */
    UNDERLYING_IV();
  ...
  /**
   * Creates an instance of Encrypted Random Access Buffer wrapping an underlying buffer. Keys for
   * key encryption and MAC generation are derived from the MasterSecret. If this is a new Encrypted
   * Random Access Buffer then keys are generated and the footer is written to the end of the
   * underlying buffer. Otherwise, the footer is read from the underlying buffer.
   *
   * @param type The encryption configuration to use
   * @param underlying The underlying buffer that will be storing the data. Must be larger than the
   *     footer size specified in type.
   * @param masterKey The MasterSecret that will be used to derive various keys.
   * @param newBuffer If true, initializes a new encrypted buffer; if false, reads existing header
   * @throws IOException If I/O errors occur
   * @throws GeneralSecurityException If cryptographic operations fail
   */
  public EncryptedRab(EncryptType type, Rab underlying, MasterSecret masterKey, boolean newBuffer)
   ...
   */
  public static Rab create(
      DataInputStream dis,
      FilenameGenerator fg,
      PersistentFileTracker persistentFileTracker,
      MasterSecret masterKey)
      throws IOException, StorageFormatException, ResumeFailedException {
    EncryptType type = EncryptType.getByBitmask(dis.readInt());
    if (type == null) {
      throw new StorageFormatException("Unknown EncryptedRandomAccessBufferType");
    }
    Rab underlying = BucketTools.restoreRabFrom(dis, fg, persistentFileTracker, masterKey);
    try {
      return new EncryptedRab(type, underlying, masterKey, false);
  ...
  /**
   * Reads and decrypts data from the buffer in a thread-safe manner. The operation uses the read
   * cipher instance with proper position tracking.
   *
  ...
  /**
   * Encrypts and writes data to the buffer in a thread-safe manner. The operation uses the write
   * cipher instance with proper position tracking.
   *
  ...
  /**
   * Closes the encrypted buffer and its underlying storage. Once closed, no further read or write
  ...
  /**
   * Handles resumption of the encrypted buffer after deserialization. Reinitializes cryptographic
   * components using the provided master secret.
   *
  ...
  public void onResume(ResumeContext context) throws ResumeFailedException {
    ...
    try {
      ...
      throw new ResumeFailedException(
          "Impossible security error resuming - maybe we " + "lost a codec?", e);
    }
  }

  /**
   * Serializes the encrypted buffer state. Writes the magic number, type bitmask, and underlying
   * buffer data.
   *
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof EncryptedRab other)) {
      return false;
    }
      ...
      tempPram =
          new ParametersWithIV(
              ...
              KeyGenUtil.deriveIvParameterSpec(
                      unencryptedBaseKey, getClass(), KdfInput.UNDERLYING_IV.input, type.encryptKey)
                  .getIV());
   ...
   *
   * <ul>
   *   <li>Initialization vector
   *   <li>Encrypted base key
   *   <li>MAC value
  ...
  /**
   * Verifies the integrity of the encryption header. Reads the IV, encrypted key, and MAC from the
   * header, then decrypts the key and verifies the MAC value.
   *
  ...
  private final Rab underlyingBuffer;
