...
/**
 * A factory implementation that creates {@link Rab} instances backed by byte arrays. This
 * implementation is suitable for in-memory operations and temporary storage.
 *
 * <p>The factory provides methods to create either empty buffers of a specified size or buffers
 * initialized with existing data. All buffers created by this factory are limited to arrays that
 * can fit in memory (maximum size of {@link Integer#MAX_VALUE}).
 *
 * @author toad
 * @see Rab
 * @see RabFactory
...
public class ByteArrayRabFactory implements RabFactory {
   ...
   * byte array.
   *
   * @param size The size of the buffer to create, must be non-negative and not larger than {@link
   *     Integer#MAX_VALUE}
   * @return A new {@link Rab} instance backed by a byte array
   * @throws IllegalArgumentException if size is negative
   * @throws IOException if the requested size exceeds {@link Integer#MAX_VALUE}
   */
  @Override
  ...
  /**
   * Creates a new {@link Rab} initialized with data from the provided byte array. The data is
   * copied from the specified range of the input array.
   *
   * <p>The resulting buffer can be created as read-only if required.
   *
   ...
   * @throws IllegalArgumentException if offset or size is negative, or if the specified range
   *     exceeds the bounds of the initial contents array
