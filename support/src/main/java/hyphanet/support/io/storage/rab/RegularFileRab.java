...
/**
 * A simple implementation of {@link Rab} that uses a regular file as the backing store.
 *
 * <p>This class provides basic random access operations for reading and writing data at specific
...
public class RegularFileRab implements Rab, Serializable {
  ...
  private static final Logger logger = LoggerFactory.getLogger(RegularFileRab.class);

  /**
   * Constructs a new {@link RegularFileRab} with the specified path, length, and read-only mode.
   *
   * @param filePath the path to the file
   * @param length the length of the file in bytes
   * @param readOnly {@code true} to open the file in read-only mode, {@code false} for read-write
   *     mode
   * @throws IOException if an I/O error occurs while opening or creating the file
   */
  public RegularFileRab(Path filePath, long length, boolean readOnly) throws IOException {
    this.length = length;
    this.path = filePath;
  ...
  /**
   * Constructs a new {@link RegularFileRab} with the specified path and read-only mode.
   *
   * <p>The length of the file is determined by reading the file's size from the file system.
   *
   * @param filePath the path to the file
   * @param readOnly {@code true} to open the file in read-only mode, {@code false} for read-write
   *     mode
   ...
   */
  public RegularFileRab(Path filePath, boolean readOnly) throws IOException {
    this(filePath, Files.size(filePath), readOnly);
  }

  /**
   * Constructs a new {@link RegularFileRab} by deserializing from the provided input stream.
   *
   * @param dis the input stream to read from
   * @throws IOException if an I/O error occurs while reading from the input stream
   * @throws StorageFormatException if the data in the input stream is not a valid {@link
   *     RegularFileRab}
   * @throws ResumeFailedException if the file specified in the input stream does not exist or has
   *     an incorrect length
   */
  public RegularFileRab(DataInputStream dis)
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof RegularFileRab other)) {
      return false;
    }
   ...
   * @throws IllegalArgumentException if the file offset is negative or if the requested read would
   ...
   * @throws IllegalArgumentException if the file offset is negative or if the requested write would
   *     exceed the file's length
   */
