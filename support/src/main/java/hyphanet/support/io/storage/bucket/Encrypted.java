 ...
 * hyphanet.support.io.storage.randomaccessbuffer.Encrypted} Random Access Buffer. This allows for seamless
 * conversion between encrypted bucket and buffer formats without data replication.
 *
...
public class Encrypted implements RandomAccessible, Serializable {
  ...
  public RandomAccessBuffer toRandomAccessBuffer() throws IOException {
    ...
    try {
      return new hyphanet.support.io.storage.randomaccessbuffer.Encrypted(type, r, masterKey, false);
    } catch (GeneralSecurityException e) {
      ...
      KeyParameter cipherKey =
          new KeyParameter(
              KeyGenUtil.deriveSecretKey(
                            unencryptedBaseKey,
                            hyphanet.support.io.storage.randomaccessbuffer.Encrypted.class,
                            hyphanet.support.io.storage.randomaccessbuffer.Encrypted.KdfInput.UNDERLYING_KEY
                          .input,
                            type.encryptKey)
                  .getEncoded());
      tempPram =
          new ParametersWithIV(
              ...
              KeyGenUtil.deriveIvParameterSpec(
                            unencryptedBaseKey,
                            hyphanet.support.io.storage.randomaccessbuffer.Encrypted.class,
                            hyphanet.support.io.storage.randomaccessbuffer.Encrypted.KdfInput.UNDERLYING_IV.input,
                            type.encryptKey)
                  .getIV());
    } catch (InvalidKeyException e) {
