...
/**
 * Temporary file handling. {@link TempFile} Buckets start empty and are typically used for
 * short-lived storage.
 *
 * <p>This class extends {@link BaseFile} and implements {@link Serializable} to support persistence
 ...
 * depending on the constructor used and configuration. However, {@link TempFile} buckets are
 ...
 */
public class TempFile extends BaseFile implements Serializable {
  ...
  /**
   * Constructs a new {@link TempFile}.
   *
   ...
   * @see #TempFile(long, FilenameGenerator, boolean)
   */
  public TempFile(long id, FilenameGenerator generator) {
  ...
  /**
   * Constructor for the {@link TempFile} object. Subclasses can call this constructor.
   *
   ...
   * @see #TempFile(long, FilenameGenerator)
   * @see #dispose()
   */
  protected TempFile(long id, FilenameGenerator generator, boolean deleteOnDispose) {
    super();
    filenameId = id;
   ...
   * reconstruct a {@link TempFile} object.
   *
   ...
   */
  protected TempFile(DataInputStream dis) throws IOException, StorageFormatException {
    super(dis);
   ...
   * {@inheritDoc}
   *
   * <p>Returns the {@link Path} associated with this {@link TempFile}. If the path is not yet
   ...
   * {@inheritDoc}
   *
   * @return {@code true} if this {@link TempFile} is read-only, {@code false} otherwise.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Sets this {@link TempFile} to read-only. Once set to read-only, this operation cannot be
   * reversed.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Creates a shallow, read-only shadow copy of this {@link TempFile}. The shadow copy shares
   * the same underlying storage and {@link #filenameId}.
   *
   * <p><strong>Warning:</strong> If the original {@link TempFile} is deleted or freed, the shadow
   * copy may become invalid and operations on it might fail.
   *
   * @return A new {@link TempFile} instance representing the shadow copy, or {@code null} if shadow
   *     creation is not supported. In this implementation, it always returns a new {@link TempFile}
   *     shadow instance.
   */
  @Override
  public RandomAccessible createShadow() {
    var ret = new TempFile(filenameId, generator, false);
    ret.setReadOnly();
   ...
   * {@inheritDoc}
   *
   * <p>Resumes the {@link TempFile} after restart or deserialization. This method is called by the
   * framework to allow the {@link TempFile} to perform any necessary initialization or recovery
   * steps.
   *
   * <p>For {@link TempFile}, it checks if the file exists and potentially moves it to the correct
   ...
   *     TempFile} cannot be properly initialized.
   ...
   * {@inheritDoc}
   *
   * <p>Stores the {@link TempFile}'s state and metadata to the provided {@link DataOutputStream}
   ...
   * {@inheritDoc}
   *
   * @return A hash code value for this {@link TempFile} object, based on {@link #deleteOnDispose},
   ...
   * {@inheritDoc}
   *
   * <p>Compares this {@link TempFile} to another object for equality. Two {@link TempFile} objects
   ...
   * {@inheritDoc}
   *
   * @return {@code false} because {@link TempFile} does not enforce creation of a new file only. It
   ...
   * {@inheritDoc}
   *
   * @return {@code false} because {@link TempFile} manages its deletion lifecycle differently,
  ...
  /**
   * Indicates whether this {@link TempFile} is persistent across restarts.
   *
   * <p>For {@link TempFile}, it always returns {@code false} as it is designed for temporary
   * storage. Subclasses like {@link PersistentTempFileBucket} may override this to return {@code
   * true}.
   *
   * @return {@code false} for {@link TempFile}, indicating it is not persistent by default.
   */
   ...
   * {@inheritDoc}
   *
   * @return {@code true} because {@link TempFile} is designed to work with existing temporary
   ...
   * their specific magic number. {@link TempFile} itself does not define a specific magic number
  ...
  /**
   * The {@link FilenameGenerator} used to generate file paths for this {@link TempFile}.
   *
  ...
  /**
   * Flag indicating whether this {@link TempFile} is read-only.
   *
  ...
  /**
   * Flag indicating whether the {@link TempFile} has been resumed after restart or deserialization.
   *
