...
/**
 * A wrapper class that provides a padded view of a RandomAccessBuffer with a specified real size.
 * This class ensures that operations cannot exceed the real size limit, even if the underlying
 * buffer is larger.
 *
 * <h2>Usage</h2>
 *
 ...
 *
 * <ul>
 *   <li>Limit access to a specific portion of a larger buffer
 *   <li>Ensure operations don't exceed a predetermined size
 *   <li>Maintain size constraints during serialization/deserialization
 * </ul>
 *
 * @see Rab
 */
public class PaddedRab extends AbstractStorage implements Rab, Serializable {
   ...
   * @throws IllegalArgumentException if realSize is negative or exceeds the underlying buffer size
   */
  public PaddedRab(Rab rab, long realSize) {
    this.rab = rab;
    if (realSize < 0) {
    ...
    if (realSize > rab.size()) {
      throw new IllegalArgumentException(
          ("Real size %d exceeds underlying buffer size" + " " + "%d")
              .formatted(realSize, rab.size()));
    }
  ...
  public PaddedRab(
    ...
    if (realSize > rab.size()) {
      throw new ResumeFailedException(
          "Padded file size %d is smaller than expected length %d".formatted(rab.size(), realSize));
    }
  }
  ...
  public void pread(long fileOffset, byte[] buf, int bufOffset, int length) throws IOException {
    ...
    if (fileOffset + length > realSize) {
      throw new IOException(
          "Read operation exceeds size limit: offset=%d, length=%d, limit=%d"
              .formatted(fileOffset, length, realSize));
    }
  ...
  public void pwrite(long fileOffset, byte[] buf, int bufOffset, int length) throws IOException {
    ...
    if (fileOffset + length > realSize) {
      throw new IOException(
          "Write operation exceeds size limit: offset=%d, length=%d, limit=%d"
              .formatted(fileOffset, length, realSize));
    }
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof PaddedRab other) {
