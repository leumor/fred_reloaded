 ...
 * <p>
 * These temporary files are encrypted using an ephemeral key (unless the node is configured
 * not to encrypt temporary files as happens with physical security level LOW). FIXME NO CRYPTO
 * AT THE MOMENT.
 * <p>
 * Note that the files are only deleted *after* the transaction containing their deletion
 * reaches disk - so we should not leak temporary files, or forget that we deleted a bucket and
 * try to reuse it, if there is an unclean shutdown.
 * <p>
 ...
 */
public class PersistentTempBucketFactory implements Factory, PersistentFileTracker {

    /**
     * Filename generator. Tracks the directory and the prefix for temp files, can move them if
     * these change, generates filenames.
     */
    ...
    /**
     * Notify the bucket factory that a file is a temporary file, and not to be deleted. FIXME
     * this is not currently used. @see #completedInit()
     */
    @Override
    public void register(File file) {
        synchronized (this) {
            ...
            file = FilePath.getCanonicalFile(file);
            if (logMINOR) {
    ...
    public RandomAccessBucket makeBucket(long size) throws IOException {
        ...
        synchronized (encryptLock) {
            if (encrypt) {
                rawBucket = new PaddedRandomAccess(rawBucket);
                rawBucket = new Encrypted(TempBucketFactory.CRYPT_TYPE, rawBucket, secret);
            }
        }
        if (mustWrap) {
            rawBucket = new DelayedDisposeRandomAccess(this, rawBucket);
        }
        return rawBucket;
    }
    ...
    /**
     * Set whether to encrypt new persistent temp buckets. Note that we do not encrypt/decrypt
     * old ones when this changes.
     */
