 ...
 * <ul>
 *   <li>Path canonicalization and normalization</li>
 *   <li>Parent-child relationship verification</li>
 *   <li>Filename sanitization</li>
 *   <li>File content comparison</li>
 * </ul>
 */
public final class FilePath {
     ...
     * <p>
     * This charset is determined by the system's file encoding configuration. On Windows and
     * Linux, it typically matches the user's system language settings. While mis-detection of
     * the filename charset may cause download failures due to invalid filenames, path and file
     * separator characters are always disallowed to prevent arbitrary file storage locations.
     */
        ...
        try {
            ...
            String normalizedPath =
                (SystemInfo.DETECTED_OS == SystemInfo.OperatingSystem.WINDOWS) ?
                    path.toString().toLowerCase() : path.toString();
     ...
     * {@link #sanitizeFileName(String, SystemInfo.OperatingSystem, String)} with no extra
     * allowed characters.
     *
     * <p>The method uses the system's detected operating system
     * ({@link SystemInfo#DETECTED_OS}) to determine which character restrictions to apply.
     * This ensures the filename will be valid on the current platform.</p>
     *
     ...
     * to {@link #sanitizeFileName(String, SystemInfo.OperatingSystem, String)}.
     *
