...
/**
 * A simple implementation of {@link RunningAverage} that maintains a running sum and count. This
 * class provides a thread-safe way to calculate running averages of reported values.
 *
 * <p>The average is calculated as the sum of all reported values divided by the number of reports.
...
public final class TrivialRunningAverage implements RunningAverage {
  ...
  /**
   * Creates a new TrivialRunningAverage with initial values set to zero. The initial state will
   * have no reports and a total of 0.0.
   */
  ...
  /**
   * Creates a new TrivialRunningAverage by copying the state from another instance. This
   * constructor creates a deep copy of the provided instance.
   *
  ...
  /**
   * Creates and returns a deep copy of this instance. The clone is thread-safe and independent of
