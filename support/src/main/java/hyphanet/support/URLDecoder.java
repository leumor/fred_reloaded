 ...
 *
 * <ul>
 *   <li>Decodes percent-encoded sequences (e.g., %20 → space, %2F → forward slash)
 *   <li>Uses UTF-8 character encoding exclusively
 *   <li>Provides strict and tolerant decoding modes
 *   <li>Prevents null byte injection attacks
 * </ul>
 *
 * <p><strong>Important Differences from {@link java.net.URLDecoder}:</strong>
 *
 * <ul>
 *   <li>Does NOT decode application/x-www-form-urlencoded format
 *   <li>Does NOT convert plus signs (+) to spaces
 *   <li>Focuses solely on URI component decoding
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>Prevents null byte injection by rejecting %00 sequences
 *   <li>Validates all percent-encoded sequences
 *   <li>Uses strict UTF-8 encoding/decoding
 *   <li>Throws exceptions for malformed input in strict mode
 * </ul>
 *
...
public final class URLDecoder {
  ...
  /**
   * Decodes a URL-encoded string using UTF-8 charset. This method handles percent-encoded sequences
   * in URLs (e.g., %20 for space, %2F for forward slash).
   *
   * <p>Example usage:
   *
   * <pre>
   ...
   *
   * <ul>
   *   <li>Regular characters are converted to UTF-8 bytes directly
   *   <li>Percent-encoded sequences (%XX) are converted to their byte values
   *   <li>Invalid or incomplete percent-encoded sequences trigger exceptions
   * </ul>
   *
   ...
   *     characters instead of throwing exceptions. This mode is useful when processing user-pasted
   *     URLs that might contain un-encoded % characters. Not recommended for security-sensitive
   *     applications.
  ...
  private static int decodeHexSequence(
      ...
      final boolean hasDecodedSomething)
