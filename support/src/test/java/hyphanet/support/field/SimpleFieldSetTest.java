...
public class SimpleFieldSetTest {
    ...
    /**
     * Tests subset(String) method putting two levels keys and fetching it through subset() method
     * on the first level and then get() on the second
     */
    @Test
    public void testSimpleFieldSetSubset_String() {
        ...
        for (int i = 0; i < methodPairsArray_MultiLevel.length; i++) {
            methodSFS.putSingle(
                methodPairsArray_MultiLevel[i][0] + SimpleFieldSet.MULTI_LEVEL_CHAR +
    ...
    public void testPut_StringSimpleFieldSet() {
        ...
        for (int i = 0; i < SAMPLE_STRING_PAIRS.length; i++) {
            assertEquals(methodSFS.get(
                             methodKey + SimpleFieldSet.MULTI_LEVEL_CHAR + SAMPLE_STRING_PAIRS[i][0]),
    ...
    /**
     * Tests put(String,boolean) and getBoolean(String,boolean) methods consistency. The default
     * value (returned if the key is not found) is set to "false" and the real value is always set
     * to "true", so we are sure if it finds the right value or not (and does not use the default).
     */
    @Test
     ...
     * default value (returned if the key is not found) is set to a not present int value, so we are
     ...
     * default value (returned if the key is not found) is set to a not present long value, so we
     * are sure if it finds the right value or not (and does not use the default).
     */
    @Test
     ...
     * default value (returned if the key is not found) is set to a not present char value, so we
     * are sure if it finds the right value or not (and does not use the default).
     */
    @Test
    ...
    /**
     * Tests put(String,short) and [getShort(String)|getShort(String,short)] methods consistency.
     * The default value (returned if the key is not found) is set to a not present short value, so
     ...
     * consistency. The default value (returned if the key is not found) is set to a not present
     * double value, so we are sure if it finds the right value or not (and does not use the
     * default).
     */
    @Test
    ...
    public void testSplit() {
        ...
        assertArrayEquals(SimpleFieldSet.split("blah;1;2;"), new String[]{"blah", "1", "2", ""});
        assertArrayEquals(SimpleFieldSet.split("blah;1;2;;"),
                          new String[]{"blah", "1", "2", "", ""});
        assertArrayEquals(SimpleFieldSet.split(";blah;1;2;;"),
                          new String[]{"", "blah", "1", "2", "", ""});
        assertArrayEquals(SimpleFieldSet.split(";;blah;1;2;;"),
    ...
    /**
     * Verifies if all keys in a String[][] (We consider that keys are stored in String[x][0]) are
     * the same that the Iterator provides. In this way both hasNext() and next() methods are
     * tested.
     *
     * @param aStringPairsArray
     * @param aPrefix           that could be put before found key
