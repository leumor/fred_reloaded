...
/**
 * Test case for {@link hyphanet.support.field.SimpleFieldSet} class.
 *
...
class SimpleFieldSetTest {
    ...
    private static final String[][] SAMPLE_STRING_PAIRS = {
        //directSubset
        {"foo", "bar"}, {"foo.bar", "foobar"}, {"foo.bar.foo", "foobar"},
        {"foo.bar.boo.far", "foobar"}, {"foo2", "foobar.fooboo.foofar.foofoo"},
    ...
    void testSimpleFieldSetPutAndGet_MultiLevel() {
        ...
        String[][] methodPairsArrayMultiLevel =
            {{"A.A.A.A", "aa"}, {"A.B.A", "ab"}, {"A.C.Cc", "ac"}, {"A.D.F", "ad"},
                {"A.E.G", "ae"}, {"A.F.J.II.UI.BOO", "af"}};
        assertTrue(checkPutAndGetPairs(methodPairsArrayDoubleLevel));
        assertTrue(checkPutAndGetPairs(methodPairsArrayMultiLevel));
    }

    /**
     * Tests subset(String) method putting two levels keys and fetching it through subset()
     * method on the first level and then get() on the second
     */
    @Test
    void testSimpleFieldSetSubset_String() {
        ...
        for (String[] strings : methodPairsArrayMultiLevel) {
            methodSFS.putSingle(strings[0] + SimpleFieldSet.MULTI_LEVEL_CHAR + strings[1],
                                strings[2]);
        }
    ...
    /**
     * Tests putAllOverwrite(SimpleFieldSet) method trying to overwrite a whole SimpleFieldSet
     * with another with same keys but different values
     */
    @Test
    void testPutAllOverwrite() {
        ...
        for (String[] sampleStringPair : SAMPLE_STRING_PAIRS) {
            assertEquals(methodSFS.get(sampleStringPair[0]),
    ...
    void testPut_StringSimpleFieldSet() {
        ...
        for (String[] sampleStringPair : SAMPLE_STRING_PAIRS) {
            assertEquals(methodSFS.get(
                             methodKey + SimpleFieldSet.MULTI_LEVEL_CHAR + sampleStringPair[0]),
                         sampleStringPair[1]);
        }
    }
    ...
    /**
     * Tests put(String,SimpleFieldSet) and tput(String,SimpleFieldSet) trying to add empty
     * data structures
     */
    @Test
     ...
     * default value (returned if the key is not found) is set to "false" and the real value is
     * always set to "true", so we are sure if it finds the right value or not (and does not
     * use the default).
     */
    @Test
    ...
    /**
     * Tests put(String,int) and [getInt(String),getInt(String,int)] methods consistency. The
     * default value (returned if the key is not found) is set to a not present int value, so
    ...
    /**
     * Tests put(String,long) and [getLong(String),getLong(String,long)] methods consistency.
     * The default value (returned if the key is not found) is set to a not present long value,
    ...
    /**
     * Tests put(String,char) and [getChar(String),getChar(String,char)] methods consistency.
     * The default value (returned if the key is not found) is set to a not present char value,
     * so we are sure if it finds the right value or not (and does not use the default).
     */
    @Test
     ...
     * present short value, so we are sure if it finds the right value or not (and does not use
     * the default).
     */
    @Test
     ...
     * consistency. The default value (returned if the key is not found) is set to a not
     * present double value, so we are sure if it finds the right value or not (and does not
     * use the default).
     */
    @Test
        ...
        for (double[] doubles : methodPairsArray) {
            try {
                ...
                assertEquals(0, Double.compare(
                    methodSFS.getDouble(Double.toString(doubles[0]), 5), doubles[1]));
            } catch (FSParseException aException) {
                fail("Not expected exception thrown : " + aException.getMessage());
            }
        }
    ...
    /**
     * Tests SimpleFieldSet(String,boolean,boolean) constructor, with simple and border cases
    ...
    /**
     * Tests SimpleFieldSet(BufferedReader,boolean,boolean) constructor, with simple and border
    ...
    /**
     * Tests SimpleFieldSet(SimpleFieldSet) constructor, with simple and border cases of the
     * canonical form.
     */
    @Test
    ...
    /**
     * Tests directSubsetNameIterator() method. It uses SAMPLE_STRING_PAIRS and for this reason
    ...
    void testPutAppend() {
        ...
        String expectedResult =
            "boo" + SimpleFieldSet.MULTI_VALUE_CHAR + "bar" + SimpleFieldSet.MULTI_VALUE_CHAR +
            "zoo";
        SimpleFieldSet methodSFS = new SimpleFieldSet(true);
        for (String methodValue : methodValues) {
    ...
    /**
     * Tests the Iterator given for the SimpleFieldSet class. It tests hasNext() and next()
     * methods.
     */
    @Test
    ...
    /**
     * Tests the toplevelIterator given for the SimpleFieldSet class. It tests hasNext() and
     * next() methods.
     * <p>
    ...
    void testSplit() {
        ...
        assertArrayEquals(new String[]{"blah", "1", "2", ""},
                          SimpleFieldSet.split("blah;1;2;"));
        assertArrayEquals(new String[]{"blah", "1", "2", "", ""},
                          SimpleFieldSet.split("blah;1;2;;"));
        assertArrayEquals(new String[]{"", "blah", "1", "2", "", ""},
                          SimpleFieldSet.split(";blah;1;2;;"));
    ...
    /**
     * It puts key-value pairs in a SimpleFieldSet and verify if it can do the correspondant
     * get correctly.
     *
    ...
    private String sfsReadyString(String[][] aStringPairsArray) {
        ...
        for (String[] strings : aStringPairsArray) {
            methodStringToReturn.append(strings[0]).append(KEY_VALUE_SEPARATOR)
    ...
    /**
     * Verifies if all keys in a String[][] (We consider that keys are stored in String[x][0])
     * are the same that the Iterator provides. In this way both hasNext() and next() methods
     * are tested.
     *
     * @param aPrefix that could be put before found key
     *
