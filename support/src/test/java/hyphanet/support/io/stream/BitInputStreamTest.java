...
class BitInputStreamTest {
  ...
  @Test
  void readAlignedBytesTest() throws IOException {
  ...
  @Test
  void readFullyTest() throws IOException {
    byte[] ba = {5, 4, 3, 2, 1};
    try (BitInputStream in = new BitInputStream(new ByteArrayInputStream(ba))) {
      byte[] a = new byte[3];
      in.readFully(a);
  ...
  @Test
  void readAlignedBytes_littleEndianTest() throws IOException {
  ...
  @Test
  void unalignedBytesTest() throws IOException {
    String bitData =
  ...
  @Test
  void unalignedBytes_littleEndianTest() throws IOException {
    try (BitInputStream in =
        new BitInputStream(
  ...
  void endOfStreamTest() throws IOException {
    byte[] ba = {0};
    try (BitInputStream in = new BitInputStream(new ByteArrayInputStream(ba))) {
      assertThrows(EOFException.class, () -> in.readInt(9));
    }
  }

  @Test
  void readFully_endOfStreamTest() throws IOException {
    byte[] ba = {0};
    try (BitInputStream in = new BitInputStream(new ByteArrayInputStream(ba))) {
      assertThrows(EOFException.class, () -> in.readFully(new byte[2]));
    }
  }

  @Test
  void readNegativeNumberOfBitsTest() throws IOException {
    byte[] ba = {0};
    try (BitInputStream in = new BitInputStream(new ByteArrayInputStream(ba))) {
      assertThrows(IllegalArgumentException.class, () -> in.readInt(-1));
    }
  }
